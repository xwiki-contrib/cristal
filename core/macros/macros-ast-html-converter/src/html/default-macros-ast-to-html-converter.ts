          ...
          block.items
            .map((item) =>
              this.produceHtmlEl(
                "li",
                {},
                `${item.checked !== undefined ? this.produceHtmlEl("input", { type: "checkbox", checked: item.checked.toString(), readonly: "true" }, false) : ""}${this.convertInlineContents(item.content, htmlBody)}`,
              ),
            )
            .join(""),
        );
